...
// POST — generate/regenerate fact sheet
export async function POST(
    ...
    try {
      ...
      if (memberResp?.member) {
        ...
        congressData = {
          ...
          chamber: latestTerm.chamber || (m.terms?.some?.((t: Record<string, unknown>) => t.chamber === 'Senate') ? 'Senate' : 'House'),
  ...
  try {
    // Fetch relevant data items for extraction
    ...
    // Fetch soul document if available
...
${sourceTexts.slice(0, 8000)}${soulContent}`,
  ...
  return NextResponse.json({ fact_sheet: factSheet });
}
