...
export default function ParticipantsPage() {
  ...
  async function handleDelete(id: string) {
    ...
    await apiFetch(`/api/events/admin/participants/${id}`, { method: 'DELETE' })
  ...
  async function handleBulkAssign() {
    ...
    await Promise.all(
      Array.from(selected).map(id =>
        apiFetch(`/api/events/admin/participants/${id}`, {
          method: 'PATCH',
  ...
  function copyLink(p: Participant) {
    const url = `${window.location.origin}/events/attendee/${p.access_token}`
