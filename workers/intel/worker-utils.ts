...
export async function callClaude(system: string, userMessage: string, maxTokens = 4096): Promise<{ text: string; inputTokens: number; outputTokens: number }> {
...
export async function getAllActiveOrgs(): Promise<{ id: string; name: string }[]> {
  const { data } = await supabase.from('intel_organizations').select('id, name');
  return data || [];
}
